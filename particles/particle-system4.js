...
function TimedLogger (timeout, func) {
  this.last = 0;
  this.timeout = timeout;
  this.update = function (t) {
    if( t >= this.last + timeout ) {
      this.last = t;
      func(this);
    }
  };
...
CanvasParticles.setup = function () {
  ...
  context.emitter3 = context.particleSystem.addEmitter(prepParticle(context.coloring3));
  context.emitter4 = context.particleSystem.addEmitter(prepParticle(context.coloring4));
  ...
  context.loggers = NIL;
  context.loggers = context.loggers.prepend(  // particle count log
    new TimedLogger(2, function () {
      console.log('ps.size: ' + context.particleSystem.active.size());
    }));
  context.loggers = context.loggers.prepend(  // frame rate log
    new TimedLogger(2, function (tl) {
      console.log('fps: ' + this.frameCount / tl.timeout);
      this.frameCount = 0;
    }));
  context.loggers = context.loggers.prepend(  // frame counter pseudo-log
    new TimedLogger(0, function () {
      this.frameCount += 1;
    }));
...
CanvasParticles.update = function () {
  ...
  this.attractor1.str = Math.sin(time / 3);
  this.attractor1.loc = new vec3(-.5, -.5, 0);
  ...
  this.attractor3.str = Math.cos(time / 3);
  this.attractor3.loc = new vec3(.3, .3, 2);

  this.emitter1.loc = new vec3(.3, 0, 0);
  this.emitter2.loc = new vec3(-.3, .3, 0);
  this.emitter3.loc = new vec3(-.3, -.3, 0);
  this.emitter4.loc = new vec3(.5, -.5, -.3);

  this.emitter1.dir = new vec3(-1, 0, 2).normalized().scale(.5);
  this.emitter2.dir = new vec3(0, -1, -2).normalized().scale(.5);
  this.emitter3.dir = new vec3(1, 1, 2).normalized().scale(.5);
  this.emitter4.dir = new vec3(-1, 1, 4).normalized().scale(.5);

  this.coloring1.update(DELTA_TIME);
  this.coloring2.update(DELTA_TIME);
  this.coloring3.update(DELTA_TIME);
  this.coloring4.update(DELTA_TIME);
